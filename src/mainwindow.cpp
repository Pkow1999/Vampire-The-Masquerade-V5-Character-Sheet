    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(discordSender, &QNetworkAccessManager::finished, this, &MainWindow::replyFinished);
}
...
void MainWindow::deleteDiscipline()
{
    ...
    if (ui->Discipline1Group->buttons().first()->isChecked())
        ui->Discipline1Group->buttons().first()->click();

    ui->Discipline2Group->buttons().first()->click();
    if (ui->Discipline2Group->buttons().first()->isChecked())
        ui->Discipline2Group->buttons().first()->click();

    ui->Discipline3Group->buttons().first()->click();
    if (ui->Discipline3Group->buttons().first()->isChecked())
        ui->Discipline3Group->buttons().first()->click();

    ui->Discipline4Group->buttons().first()->click();
    if (ui->Discipline4Group->buttons().first()->isChecked())
        ui->Discipline4Group->buttons().first()->click();
}
...
void MainWindow::calculateWP()
{
    ...
    willpowerPool = statsCounter->countDots(ui->Com) + statsCounter->countDots(ui->Res)
                    + ui->wpModifier->value();
    if (willpowerPool < 0)
        willpowerPool = 0;
...
void MainWindow::calculateBlood()
{
    ...
    ui->MendAmount->setText(
        QString::number(bloodPotency < 6 ? bloodPotency / 2 + 1 : bloodPotency / 2));
    ui->PowerBonus->setText(QString::number(bloodPotency / 2));
    ui->RouseReRoll->setText(
        tr("Level ") + QString::number(bloodPotency > 0 ? bloodPotency / 2 + bloodPotency % 2 : 0));
    ui->BaneSevarity->setText(
        QString::number(bloodPotency > 0 ? bloodPotency / 2 + bloodPotency % 2 + 1 : 0));
...
void MainWindow::clear()
{
    ...
    {
        QAbstractButton *but = qobject_cast<QAbstractButton *>(
            findParentLayout(bt)
                ->itemAt(1)
                ->layout()
                ->itemAt(0)
                ->widget()); //magiczny syf do wyciagniecia przycisku z atrybutami
    ...
    for (QAbstractButton *bt : ui->MentalSkillsGroup->buttons()) //skille
    {
        ...
        line->clear();
        QAbstractButton *but = qobject_cast<QAbstractButton *>(
            findParentLayout(bt)->itemAt(2)->layout()->itemAt(0)->widget());
        for (QAbstractButton *dot : but->group()->buttons()) {
            dot->setChecked(false);
        }
    }
    ...
    if (ui->diceModifier->value()) {
        if (!statsCounter->getPoolNames()->empty())
            statsCounter->getPoolNames()->append(
                tr("Modifier %1").arg(QString::number(ui->diceModifier->value())));
        else
            statsCounter->getPoolNames()->append(
...
int MainWindow::calculatePool()
{
    ...
    for (int i = 0; i < ui->buttonGroup->buttons().size(); i++) {
        if (ui->buttonGroup->buttons().at(i)->isChecked()) {
            ...
            QAbstractButton *bt = qobject_cast<QAbstractButton *>(
                findParentLayout(ui->buttonGroup->buttons().at(i)->focusWidget())
                    ->layout()
                    ->itemAt(1)
                    ->layout()
                    ->itemAt(0)
    ...
    for (int i = 0; i < ui->MentalSkillsGroup->buttons().size(); i++) {
        if (ui->MentalSkillsGroup->buttons().at(i)->isChecked()) {
            ...
            QAbstractButton *bt = qobject_cast<QAbstractButton *>(
                findParentLayout(ui->MentalSkillsGroup->buttons().at(i)->focusWidget())
                    ->layout()
                    ->itemAt(2)
                    ->layout()
                    ->itemAt(0)
    ...
    for (int i = 0; i < ui->SocialSkillsGroup->buttons().size(); i++) {
        if (ui->SocialSkillsGroup->buttons().at(i)->isChecked()) {
            //znajdujemy dokladnego parrenta naszego przycisku, nastepnie sprawdzamy jaki jest trzeci(!) element (ktorym sa radio buttony) i z tych radio buttonow przechodzimy na do grupy ktora tworza aby zliczyc ile sie swieci
            QAbstractButton *bt = qobject_cast<QAbstractButton *>(
                findParentLayout(ui->SocialSkillsGroup->buttons().at(i)->focusWidget())
                    ->layout()
                    ->itemAt(2)
                    ->layout()
                    ->itemAt(0)
    ...
    for (int i = 0; i < ui->PhysicalSkillsGroup->buttons().size(); i++) {
        if (ui->PhysicalSkillsGroup->buttons().at(i)->isChecked()) {
            //znajdujemy dokladnego parrenta naszego przycisku, nastepnie sprawdzamy jaki jest trzeci(!) element (ktorym sa radio buttony) i z tych radio buttonow przechodzimy na do grupy ktora tworza aby zliczyc ile sie swieci
            QAbstractButton *bt = qobject_cast<QAbstractButton *>(
                findParentLayout(ui->PhysicalSkillsGroup->buttons().at(i)->focusWidget())
                    ->layout()
                    ->itemAt(2)
                    ->layout()
                    ->itemAt(0)
    ...
    for (int i = 0; i < ui->buttonGroup_3->buttons().size(); i++) {
        if (ui->buttonGroup_3->buttons().at(i)->isChecked()) {
            //znajdujemy dokladnego parrenta naszego przycisku, nastepnie sprawdzamy jaki jest trzeci(!) element (ktorym sa radio buttony) i z tych radio buttonow przechodzimy na do grupy ktora tworza aby zliczyc ile sie swieci
            QAbstractButton *bt = qobject_cast<QAbstractButton *>(
                findParentLayout(ui->buttonGroup_3->buttons().at(i)->focusWidget())
                    ->layout()
                    ->itemAt(2)
                    ->layout()
                    ->itemAt(0)
            ...
            QLineEdit *disciplineNameLine = qobject_cast<QLineEdit *>(
                findParentLayout(ui->buttonGroup_3->buttons().at(i)->focusWidget())
                    ->layout()
                    ->itemAt(1)
                    ->widget());
...
QJsonObject MainWindow::saveDiscipline()
{
    ...
    for (QAbstractButton *bt : ui->buttonGroup_3->buttons()) {
        ...
        QAbstractButton *butDots = qobject_cast<QAbstractButton *>(
            findParentLayout(bt)->itemAt(2)->layout()->itemAt(0)->widget());
        QLineEdit *line = qobject_cast<QLineEdit *>(findParentLayout(bt)->itemAt(1)->widget());
...
QJsonObject MainWindow::saveRest()
{
    ...
    healthPoints["superficial"] = QString::number(
        statsCounter->countIndicators(ui->Health, healthPool).first);
    healthPoints["agravated"] = QString::number(
        statsCounter->countIndicators(ui->Health, healthPool).second);
    ...
    willpowerPoints["superficial"] = QString::number(
        statsCounter->countIndicators(ui->Willpower, willpowerPool).first);
    willpowerPoints["agravated"] = QString::number(
        statsCounter->countIndicators(ui->Willpower, willpowerPool).second);
...
bool MainWindow::loadRest(QJsonObject json)
{
    if (json.contains("Rest") && json["Rest"].isArray()) {
        ...
        if (restArray.first()[hungerLabel->text()].isString()) {
            ...
            if (dots > 0) {
                for (QAbstractButton *but :
                     ui->Hunger
                         ->buttons()) //CZEMU TO DIALA A NIE DZIALA ui->Hunger->button(dots - 1) ?!?!?!!?
        ...
        if (restArray.first()["Stains"].isString()) {
            ...
            for (int i = 0; i < stains; i++) {
                QCheckBox *czek = qobject_cast<QCheckBox *>(
                    ui->HumanityLayout->itemAt(9 - i)->widget());
    ...
    if (json.contains("Attributes") && json["Attributes"].isArray()) {
        ...
        for (QAbstractButton *bt : ui->buttonGroup->buttons()) {
            if (attributesArray.first()[bt->text()].isString()) {
                ...
                if (dots > 0) {
                    QAbstractButton *but = qobject_cast<QAbstractButton *>(
                        findParentLayout(bt)->itemAt(1)->layout()->itemAt(dots - 1)->widget());
                    but->click();
                }
            }
        }
        ...
        for (QAbstractButton *bt : ui->MentalSkillsGroup->buttons()) {
            if (array.first()[bt->text()].isArray()) {
                ...
                if (dots > 0) {
                    ...
                    QAbstractButton *but = qobject_cast<QAbstractButton *>(
                        lay->itemAt(2)->layout()->itemAt(dots - 1)->widget());
                    but->click();
                }
            }
        }
        ...
        for (QAbstractButton *bt : ui->buttonGroup_3->buttons()) {
            ...
            if (array.first()[key].isArray()) {
                ...
                if (discipline.first()["dots"].isString()) {
                    ...
                    if (dots > 0) {
                        QAbstractButton *but = qobject_cast<QAbstractButton *>(
                            findParentLayout(bt)->itemAt(2)->layout()->itemAt(dots - 1)->widget());
                        but->click();
                        QLayout *lay = bt->parentWidget()->layout()->itemAt(1)->layout();
...
bool MainWindow::androidLoad(QJsonObject json)
{
    ...
    if (json.contains("Physical Skills") && json["Physical Skills"].isObject()) {
        ...
        for (QAbstractButton *bt : ui->PhysicalSkillsGroup->buttons()) {
            if (physicalSkillsJson.contains(bt->text())
                && physicalSkillsJson[bt->text()].isObject()) {
    ...
    if (json.contains("Personal Data") && json["Personal Data"].isObject()) {
        ...
        if (personalDataJson.contains("Blood Potency")
            && personalDataJson["Blood Potency"].isString()) {
...
void MainWindow::
    humanityChanged() //nienawidze tego, ale nie chce mi się ogarniać algorytmu do tego, mam wrazenie ze to by zajelo zbyt duzo czasu
{
    unsigned short hum = statsCounter->countIndicators(ui->HumanityLayout, 10).second;
    QString text;
    if (hum == 10) {
        text = tr(
            "You don't need Blush of Life.\nYou can have sex.\nYou can keep watch at day like "
            "mortal.\nYou can eat normal food.\nDamage from sunlight are split by half.");
    } else if (hum == 9) {
        text = tr("You don't need Blush of Life.\nYou can have sex.\nYou can awaken 1 hour before "
                  "the sunrise and keep watch 1h after sunset.\nYou can eat raw meat and drink.\n");
    } else if (hum == 8) {
        text = tr("You roll 2 dices for Blush of Life.\nYou can have sex with Blush of Life.\nYou "
                  "can drink wine with Blush of Life.\nYou can awaken 1 hour before the sunrise.");
    } else if (hum == 7) {
        text = tr("You can not have sex but you can fake it with Dexterity + Charisma.\nUnless you "
                  "use Blush of Life foods and drinks makes you vomit.");
    } else if (hum == 6) {
        text = tr("You can not have sex but you can fake it with Dexterity + Charisma with 1 die "
                  "penalty.\nEven with Blush Of Life foods and drinks make you vomit.");
    } else if (hum == 5) {
        text = tr("You can not have sex but you can fake it with Dexterity + Charisma with 2 dice "
        ...
        text = tr(
            "You have become the beast, you have no longer have control over your character.");
    }
...
void MainWindow::closeEvent(QCloseEvent *event)
{
    if (MainWindow::windowTitle().toStdString()[MainWindow::windowTitle().toStdString().size() - 1]
        ...
        int ret = QMessageBox::question(
            this,
            tr("Warning"),
            tr("Character Sheet has been modified.\nDo you want to save your changes?"),
...
void MainWindow::on_actionGenerate_new_Random_Character_triggered()
{
    int ret = QMessageBox::information(
        this,
        tr("Warning"),
        tr("This option will generate a completely new character!\n All previous informations will "
    ...
    QList<QAbstractButton *> bagWithAttributes = QList<QAbstractButton *>(
        ui->buttonGroup->buttons());

    QList<QAbstractButton *> bagWithSkills = QList<QAbstractButton *>(
        ui->MentalSkillsGroup->buttons());
    ...
    if (msgBox.clickedButton() == abortButton) {
        ...
        for (auto &atrValue : LUT_attributes) {
            ...
            QAbstractButton *but = qobject_cast<QAbstractButton *>(
                findParentLayout(bagWithAttributes.takeAt(QRandomGenerator::global()->generate()
                                                          % bagWithAttributes.count()))
                    ->itemAt(1)
                    ->layout()
                    ->itemAt(atrValue - 1)
                    ->widget());
        ...
        for (auto &skillValue : LUT_skills) {
            ...
            QAbstractButton *but = qobject_cast<QAbstractButton *>(
                findParentLayout(bagWithSkills.takeAt(QRandomGenerator::global()->generate()
                                                      % bagWithSkills.count()))
                    ->itemAt(2)
                    ->layout()
                    ->itemAt(skillValue - 1)
...
void MainWindow::connectAllButtons()
{
    ...
    connect(ui->Discipline1Group,
            &QButtonGroup::buttonClicked,
            this,
            &MainWindow::dynamicRemoveDots);
    connect(ui->Discipline1Group,
            &QButtonGroup::buttonClicked,
            this,
            &MainWindow::dynamicDisciplineCreator);
    connect(ui->Discipline2Group,
            &QButtonGroup::buttonClicked,
            this,
            &MainWindow::dynamicRemoveDots);
    connect(ui->Discipline2Group,
...
void MainWindow::replyFinished(QNetworkReply *reply)
{
    if (reply->error()) {
        ...
        QMessageBox warningBox;
        warningBox.setText(
            tr("There was a problem with Discord Webhook.\n%1").arg(reply->errorString()));
        warningBox.exec();
    }
    reply->deleteLater();
}
...
void MainWindow::on_frenzyRoll_button_clicked()
{
    ...
    int remainingWillpower = resolveNumber + composureNumber + willpowerModifier
                             - willpowerAggravatedDMG - willpowerSuperficialDMG;

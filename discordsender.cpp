...
DiscordSender::DiscordSender(QObject *parent, DiscordConfig *config, StatsCounter *counter) : QNetworkAccessManager(parent), discordConfig(config), statsCounter(counter)
{

}

QNetworkReply *DiscordSender::sendData(const QString& poolFormatted, const QString& normalDicesFormatted, const QString& hungerDicesFormatted){
    ...
    postData.append(
        QString(
            "{\"embeds\": [{"
            "\"title\":" + tr("\"%1 rolls:\",").arg(discordConfig->getUsername()) +
            "\"description\": \"%1\","
            "\"fields\": ["
            "{"
            ...
            + (discordConfig->getUseSuccessCounter() ? ",{"
                            "\"name\":" + tr("\"%1 Successes\",").arg(QString::number(statsCounter->getLastSuccesesCount()))+
                                "\"value\": \"%5\""
                                "}" : "") +
            "]"
            "}]"
